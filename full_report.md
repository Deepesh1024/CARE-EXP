...
### Cross-Layer & Calibration Comparison
| Network Region | Sample Budget | Bivariate Scatter Distribution | Analytical Conclusion |
|---|---|---|---|
| **Early Layer (`first`)** | $N=256$ | ![Weight Distance (First, N=256)](results/exp1/256_segmented/first/scatter_first_Weight_Distance.png) | Weight distance sets an outer degradation boundary, yet low parameter Euclidean distance still exhibits unpredictable Oracle KL variance. |
| **Middle Layer (`middle`)** | $N=128$ | ![Weight Cosine (Middle, N=128)](results/exp1/128_segmented/middle/scatter_middle_Weight_Cosine.png) | High weight direction cosine alignment (>0.85) produces diffuse vertical scatter; directional similarity fails to safeguard semantic capability. |
| **Late Layer (`last`)** | $N=64$ | ![Activation Sim (Last, N=64)](results/exp1/64_segmented/last/scatter_last_Activation_Similarity.png) | In terminal readout layers, intermediate activation matching shows complete orthogonality to merge destruction. |
...
| Analytical View | Figure Embed | Insights |
|---|---|---|
| **XGBoost Gain Ranking** | ![XGBoost Gain Importance](results/exp1_5/figures/03_xgboost_importance.png) | **Relative Depth** emerges as the single most dominant splitting gain variable (0.220), confirming depth acts as a non-additive gating modulator for similarity rules. |
| **SHAP Marginal Value** | ![SHAP Summary Importance](results/exp1_5/figures/03b_shap_importance.png) | While depth guides tree branching, **Weight Distance** provides the highest actual per-prediction marginal magnitude impact ($|\text{SHAP}| = 0.00105$). |
| **LASSO Linear Weights** | ![LASSO Coefficient Profile](results/exp1_5/figures/02_lasso_coefficients.png) | Under linear L1 regularization, Weight Distance, Output Similarity, and Usage Frequency dominate, while Activation Similarity is assigned near-zero utility. |
...
| Predictive Behavior | Visual Diagnostics | Pathological Discovery |
|---|---|---|
| **Predicted vs. Oracle Scatter** | ![Predicted vs Actual Scatter](results/exp1_5/figures/04_predicted_vs_oracle.png) | **Severe mean regression and compression.** Model predictions compress tightly within $[0.001, 0.006]$. High-drift catastrophic pairs ($\text{KL} > 0.01$) are consistently underestimated. |
| **Residual Fan Pattern** | ![Residual Distribution Plot](results/exp1_5/figures/05_residual_plot.png) | Strong heteroscedastic fan pattern with extreme negative residuals along the upper prediction edge. The current pre-merge feature space lacks the explanatory variables required to predict high-drift outliers. |
