...
@torch.no_grad()
def collect_current_logits(model, eval_chunks, batch_size, max_eval_batches=None, device="cuda:0"):
    """
    Computes current model logits for marginal KL computation.
    """
    model.eval()
    current_logprobs = []
    ...
    for i in tqdm(range(num_batches), desc="Precomputing Current State Logits", leave=False):
...
@torch.no_grad()
def evaluate_marginal_damage(model, eval_chunks, current_logprobs, batch_size, max_eval_batches=None, device="cuda:0"):
    """
    Evaluates token-level marginal KL divergence of the current physically modified model against cached current_logprobs.
    """
    model.eval()
    total_kl = 0.0
    total_tokens = 0
    ...
    for i in tqdm(range(num_batches), desc="Evaluating Candidate (KL)", leave=False):
        batch = eval_chunks[i * batch_size : (i + 1) * batch_size]
        input_ids = torch.stack([x["input_ids"] for x in batch]).to(device)
        attention_mask = torch.stack([x["attention_mask"] for x in batch]).to(device)
