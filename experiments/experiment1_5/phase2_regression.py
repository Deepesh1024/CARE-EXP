"""
CARE-MoE Experiment 1.5 — Phase 2: Linearization Gap
======================================================
Train linear and nonlinear models on three feature variants (A/B/C) and
compute the Linearization Gap — the primary deliverable of Exp 1.5.

Models
------
    Linear Regression, Ridge, LASSO  (linear family)
    XGBoost                          (nonlinear ceiling)

Feature Variants
----------------
    A : 13 base features (global)
    B : 13 base + Relative_Depth          (14 features)
    C : 14 features + 13 interaction terms (27 features)

Outputs
-------
    results/exp1_5/models/{model}_{variant}.pkl
    results/exp1_5/metrics.json
    stdout: performance table and linearization gap
"""
...
def build_feature_variants(df: pd.DataFrame):
    """Build the three feature sets from a (scaled) DataFrame.

    Returns dict of {"A": np.ndarray, "B": np.ndarray, "C": np.ndarray}
    and a parallel dict of column-name lists.
    """
    # ── Variant A: Global features only ──
    cols_a = list(FEATURES)
    X_a = df[cols_a].values

    # ── Variant B: Global + Relative Depth ──
    ...
    # ── Variant C: Global + Depth + Interaction (feature × depth) ──
    depth = df["Relative_Depth"].values.reshape(-1, 1)
    interaction_cols = [f"{f}_x_depth" for f in FEATURES]
    interactions = df[FEATURES].values * depth
    ...
    for k, v in variants.items():
        print(f"[Phase 2] Variant {k}: {v.shape[1]} features, {v.shape[0]} samples")
...
def train_and_evaluate(
    ...
    """Train every model × variant combination and return structured results.

    Returns
    -------
    all_metrics : list[dict]
        One entry per (model, variant) with evaluation metrics.
    trained_models : dict
        Keyed by "{model_name}_{variant}" → fitted estimator.
    predictions : dict
        Keyed by "{model_name}_{variant}" → y_pred array.
    """
    all_metrics = []
    trained_models = {}
    predictions = {}
...
# Linearization Gap
# ──────────────────────────────────────────────
def compute_linearization_gap(all_metrics: list[dict]) -> dict:
    """Compute Δ_gap = ρ(best tree) − ρ(best linear).
...
# Pretty Table
# ──────────────────────────────────────────────
def print_performance_table(all_metrics: list[dict]) -> str:
    """Print and return a formatted markdown performance table."""
    df = pd.DataFrame(all_metrics)
    df = df[["Model", "Variant", "N_Features", "Spearman", "Pearson", "MAE", "RMSE", "R2"]]
    df = df.sort_values(["Variant", "Model"]).reset_index(drop=True)
...
def main():
    ...
    print(f"[Phase 2] Tree engine: {_TREE_ENGINE}")
    ...
    print(f"[Phase 2] Loaded train={len(train_df):,}  test={len(test_df):,}")
    ...
    # Performance table
    perf_table = print_performance_table(all_metrics)
    ...
    output = {
        ...
        "performance_table_md": perf_table,
        "predictions": pred_arrays,
        "y_test": y_test.tolist(),
    }
    save_json(output, METRICS_PATH)

    print(f"\n[Phase 2] All artifacts saved to {MODELS_DIR}")
